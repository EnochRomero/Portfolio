...
from water_flow import pressure_gain_from_water_height, water_column_height, pressure_loss_from_pipe, pressure_loss_from_fittings, reynolds_number, pressure_loss_from_pipe_reduction, psi_to_kpa
import pytest


def test_water_colume_height():
...
# Test function to test the pressure gain from the water height
...
def test_psi_to_kpa():
    ...
    assert psi_to_kpa(0) == pytest.approx(0, abs = 0.001)
    assert psi_to_kpa(101.325) == pytest.approx(14.695948775, abs = 0.001)
    assert psi_to_kpa(500) == pytest.approx(72.51886885, abs = 0.001)
